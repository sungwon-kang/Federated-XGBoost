...
class GradientBoostingMultiClassifier(BaseGradientBoosting):
    ...
    def get_tree_order(self, args, client_set):
        if args.joint == True:
            ...
            if args.method == "GinO":
                ...
                client_idx = np.repeat(client_idx, self.max_depth).reshape(len(client_idx), self.max_depth)
    ...
    def fit(self, client_set, test_set, args):
        ...
        for iter in range(0, self.n_trees):
            ...
            self.trees.append(Tree(X[sub_index], Y[sub_index], y_pred[sub_index], self.max_depth, self.min_samples_split,
        ...
        np.savetxt(f'./results/{args.seed}_all_nodes_tree.txt', np.array(counts), delimiter=' ', fmt='%f')

    def predict(self, X):
        y_pred = np.zeros((X.shape[0], self.outdim), dtype=np.float64)

        for tree in self.trees:
            y_pred += np.apply_along_axis(tree.root_node.get_predict_value, axis=1, arr=X)
    ...
    def adjust(self, client_set, iter):
        ...
        for i, subset in enumerate(client_set):
            X = subset[0]
            Y = subset[1]
            ...
            client_set[i] = (X, Y, y_pred)
        self.trees[iter].root_node.calculate_gradients(client_set, self.learning_rate)
