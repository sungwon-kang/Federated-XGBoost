...
cdef class MultinomialDeviance:
    ...
    def softmax(self, np.ndarray[DTYPE_t, ndim=2] x):
        cdef np.ndarray[DTYPE_t, ndim=2] exp_x
        exp_x = np.exp(x - np.max(x, axis=1, keepdims=True))
        return exp_x / np.sum(exp_x, axis=1, keepdims=True)

    def softmax_1d(self, np.ndarray[DTYPE_t, ndim=1] x):
        cdef np.ndarray[DTYPE_t, ndim=1] exp_x
    ...
    def get_grad_hessian(self,
        ...
        cdef np.ndarray[DTYPE_t, ndim=2] y_pred = self.softmax(y_prob)
    ...
    def getInverseMat(self,
        ...
        try:
            lamb_matrix = np.eye(self.outdim) * self.lamb  # matrix * scalar
    ...
    def find_best_split(self, tuple args):
        ...
        for feature in features:
            ...
            for fea_val in sorted_values:
                ...
                print(
                            f'------feature: %d, Split value: %.3f, Left node gain: %.3f, Right node gain: %.3f, Total gain: %.3f' %
